...
from dealerly.config import (
    LOW_FLIP_BUY_PRICE_MAX,
    LOW_FLIP_CAPITAL_MAX,
    LOW_FLIP_EFFECTIVE_TARGET_FLOOR,
    LOW_FLIP_EFFECTIVE_TARGET_MULT,
...
def effective_buy_target(target: float, buy_price: float, capital: float) -> float:
    """
    Slightly lower profit bar for cheap inventory when capital is under £5k —
    small absolute profits on £800–£2.5k buys are still attractive vs margin %.
    """
    if capital <= 0 or buy_price <= 0:
        return target
    if capital < LOW_FLIP_CAPITAL_MAX and buy_price <= LOW_FLIP_BUY_PRICE_MAX:
        relaxed = max(LOW_FLIP_EFFECTIVE_TARGET_FLOOR, target * LOW_FLIP_EFFECTIVE_TARGET_MULT)
        return min(target, relaxed)
    return target
...
def classify_decision(
    ...
    *,
    ...
    Args:
        ...
        target:    minimum profit required for BUY (before low-flip relaxation)
        shock:     worst_case_repair / capital
        threshold: capital-dependent max allowed shock ratio
        buy_price: listed price — used for low-capital flip relaxation
        capital:   available capital — used for low-capital flip relaxation
    ...
    buy_target = effective_buy_target(target, buy_price, capital)
    if profit >= buy_target:
        if buy_target < target - 0.01:
            return (
                "BUY",
                "Meets relaxed margin target for sub-£2.5k buy (small capital flip).",
            )
