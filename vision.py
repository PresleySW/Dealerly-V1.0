...
def _download_image_bytes(url: str) -> Optional[bytes]:
    ...
    Uses browser-like headers because CDNs often block bot user-agents or
    require a plausible Referer (eBay, Facebook, Motors).
    """
    u = (url or "").lower()
    if "facebook.com" in u or "fbcdn.net" in u:
        referer = "https://www.facebook.com/"
    elif "motors.co.uk" in u:
        referer = "https://www.motors.co.uk/"
    else:
...
def rank_images_for_anpr(urls: List[str]) -> List[str]:
    ...
    def _score(url: str, idx: int) -> float:
        ...
        if any(kw in u for kw in _ANPR_DEPRIORITISE):
            score -= 0.6
        # eBay CDN: larger assets read plates better (s-l1600 etc.)
        if any(x in u for x in ("s-l1600", "s-l1200", "s-l960", "s-l800")):
            score += 0.28
        elif any(x in u for x in ("s-l500", "s-l400")):
            score += 0.12
        return score
