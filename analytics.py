...
Depends on:
  ...
  - dealerly.db (insert_price_observation, load_price_observations,
    count_prior_observations_for_item)
  - dealerly.ebay (vehicle_key_from_title, guess_make_model)
  - dealerly.utils (median)

``record_price_observations`` returns (total, new_listings, repeat_listings) for the
UK stats map and console logging. No external API calls in this module.
"""
...
def record_price_observations(
    ...
    Returns (total_recorded, new_listings, repeat_listings) where *new* means
    first time this (item_id, platform) appears in price_observations; *repeat*
    is a re-sighting of an already-tracked listing (same run can still log a new row).
    """
    total = 0
    new_n = 0
    repeat_n = 0
    for listing in listings:
        try:
            ...
            prior = count_prior_observations_for_item(
                conn, listing.item_id, listing.platform,
            )
            if prior > 0:
                repeat_n += 1
            else:
