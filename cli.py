"""
dealerly/cli.py — v1.0.0-rc.1

v1.0.0-rc.1 changes:
  - Added --log-trade: interactive CLI to record a completed flip outcome
  - Added --trades: print accuracy summary of all logged trades
  - Added --import-trades <csv>: bulk import from dealer/personal CSV
  - Added --seed-demo-trades: seed Meriva KT06YNX SWAutos flip (idempotent)
  - Added --stats-map: open the UK 3D stats board (pipeline run history)
  - Quickstart now shows real-flip calibration insight from completed_trades
v0.9.4 changes:
...
from dealerly.utils import console_safe, load_dotenv, prompt_choice, prompt_float, prompt_int
...
def _open_url_preferred(url: str) -> None:
    ...
    if enable in {"1", "true", "yes"} and _antigravity_cli_path().exists():
        try:
            subprocess.Popen(
...
def _web_setup_config() -> Config | None:
    ...
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            ...
            _im = (defaults.get("input_mode") or "all").strip().lower()
            input_mode_opts = "".join(
                f'<option value="{val}"{" selected" if val == _im else ""}>{label}</option>'
                for val, label in (
                    ("all", "All platforms"),
                    ("ebay", "eBay only"),
                    ("motors", "Motors only"),
...
.wrap{{max-width:900px;margin:0 auto;background:#111827;border:1px solid #1f2937;border-radius:14px;padding:22px;box-sizing:border-box}}
h1{{margin:0 0 10px 0}} .sub{{color:#94a3b8;margin-bottom:18px}}
/* minmax(0,1fr) + min-width:0 lets range inputs shrink without overflowing the column */
.grid{{display:grid;grid-template-columns:1fr;gap:18px 20px;align-items:start}}
@media (min-width:560px){{.grid{{grid-template-columns:repeat(2,minmax(0,1fr))}}}}
.grid > div{{min-width:0}}
label{{display:block;font-size:.85rem;color:#93c5fd;margin-bottom:6px}}
input,select{{width:100%;max-width:100%;padding:9px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;box-sizing:border-box}}
.readout{{display:block;width:100%;max-width:100%;box-sizing:border-box;padding:9px 11px;border-radius:8px;border:1px solid #334155;background:#0f172a;color:#e2e8f0;font-weight:700;margin-bottom:8px}}
input[type=range]{{width:100%;max-width:100%;height:32px;margin:0 0 12px 0;padding:0;box-sizing:border-box;cursor:pointer;accent-color:#22c55e}}
.toggles{{display:grid;grid-template-columns:1fr;gap:12px;margin-top:22px;clear:both}}
@media (min-width:640px){{.toggles{{grid-template-columns:repeat(2,minmax(0,1fr))}}}}
@media (min-width:860px){{.toggles{{grid-template-columns:repeat(3,minmax(0,1fr))}}}}
.toggle{{display:flex;gap:10px;align-items:flex-start;background:#0f172a;border:1px solid #334155;border-radius:10px;padding:12px 14px;min-width:0;box-sizing:border-box}}
.toggle input{{flex-shrink:0;width:18px;height:18px;margin-top:3px;accent-color:#22c55e;cursor:pointer}}
.toggle label{{margin:0;color:#cbd5e1;font-size:.88rem;line-height:1.35;cursor:pointer;flex:1;min-width:0}}
.help{{font-size:.79rem;color:#93a3b8;margin-top:12px;line-height:1.4}}
...
<div class="grid">
  ...
  <div><label>Input Mode</label><select name="input_mode">{input_mode_opts}</select></div>
...
<div class="toggles">
  ...
  <div class="toggle"><input type="checkbox" id="use-at" name="use_autotrader" value="1" checked><label for="use-at">Use AutoTrader comps (higher confidence, slower)</label></div>
  ...
  <div class="toggle"><input type="checkbox" id="auction-only" name="auction_only" value="1"><label for="auction-only">eBay: auction listings only (timed auctions; hides most Buy It Now)</label></div>
</div>
<div class="help">Tip: AutoTrader improves confidence but increases runtime. <strong>eBay auction-only</strong> applies to eBay search; Facebook/Motors unchanged. <strong>Input mode</strong> must be <strong>All platforms</strong> for Facebook to run — <strong>Motors only</strong> / <strong>eBay only</strong> exclude other sources (no FB in the log). Facebook needs <code>fb_cookies.json</code> — run <code>python -m dealerly.facebook_setup</code> if you get 0 FB results. PowerShell: <code>$env:DEALERLY_FB_HEADLESS = &quot;0&quot;</code> if headless Chrome is blocked.</div>
...
(() => {{
  ...
  sync();
    ...
    auction_only = result.get("auction_only") == "1"
    ...
    return Config(
        ...
        ai_backend=ai_back if ai_back in ("openai", "claude", "none", "local") else "openai",
...
def main() -> None:
    ...
    # --stats-map: open the UK 3D stats board (pipeline run history from SQLite)
    if "--stats-map" in sys.argv:
        init_db(DB_PATH)
        conn = db_connect(DB_PATH)
        try:
            from dealerly.report import write_uk_stats_map_page

            out_path = write_uk_stats_map_page(conn)
            print(f"\n[Report] UK stats board written to:\n  {out_path}\n")
            try:
    ...
    # --log-trade: interactive CLI to record a completed flip outcome
    if "--log-trade" in sys.argv:
        from dealerly.trades import log_trade_interactive
        log_trade_interactive()
        return

    # --trades: print accuracy summary of all logged trades
    if "--trades" in sys.argv:
        from dealerly.trades import print_trades_summary
        print_trades_summary()
        return

    # --import-trades <csv>: bulk-import completed trades from CSV
    _import_trades_path = _parse_flag_value("--import-trades")
    if _import_trades_path:
        from dealerly.trades import import_trades_from_csv
        n = import_trades_from_csv(_import_trades_path)
        print(f"\n  [OK] Imported {n} trade(s) from {_import_trades_path}.")
        print("  Run --trades to see the updated accuracy summary.\n")
        return

    # --seed-demo-trades: seed Meriva KT06YNX SWAutos flip (idempotent)
    if "--seed-demo-trades" in sys.argv:
        from dealerly.trades import seed_demo_trades
        seeded = seed_demo_trades()
        if seeded:
            print("\n  [OK] Seeded Meriva KT06YNX (SWAutos flip). Run --trades to view.\n")
        else:
            print("\n  KT06YNX already in completed_trades — nothing to seed.\n")
        return
    ...
    if "--no-facebook" in sys.argv:
        os.environ["DEALERLY_NO_FACEBOOK"] = "1"
        print("  [--no-facebook] Facebook Marketplace adapter disabled — eBay + Motors only.")

    # Sprint 16: --agent / --agents enables the Claude-driven adaptive search loop.
    if "--agent" in sys.argv or "--agents" in sys.argv:
        cfg = dc_replace(cfg, agent_mode=True)
        print("  [--agent] Agent mode enabled — Claude will drive Phase 1 search strategy.")
...
def show_watchlist(conn) -> None:
    ...
    for w in items:
        t = console_safe(str(w.get("title") or "")[:70])
        print(f"\n  [{w['id']}] {t}")
        print(f"       Buy: {w['buy_price']:.0f} | Max bid: {w['max_bid']:.0f}"
              f" | Profit: {w['expected_profit']:.0f} | {w['decision']}")
        print(f"       VRM: {console_safe(str(w['vrm'] or '-'))} | {console_safe(str(w.get('url') or ''))}")
...
def quickstart_config() -> Config:
    ...
    # v1.0: also calibrate from real flip outcomes in completed_trades
    try:
        from dealerly.calibration import calibrate_from_trades as _cal_trades
        _conn = db_connect(DB_PATH)
        _tc = _cal_trades(_conn, capital=cap)
        _conn.close()
        _n = _tc.get("rows_analysed", 0)
        if _n >= 1:
            _avg = _tc.get("avg_realised", 0.0)
            _margin_pct = _tc.get("resale_margin_pct", 0.0)
            print(f"  [real flips] {_n} completed trade(s) -> "
                  f"avg realised £{_avg:,.0f}  "
                  f"sell/buy margin {_margin_pct:+.0%}")
            if _n >= 3 and _margin_pct > 0.05:
                _suggested = round(cap * _margin_pct * 0.8, -1)
                if 100.0 < _suggested < margin * 1.5:
                    print(f"    -> Real-flip data suggests target ~\u00a3{_suggested:.0f} "
                          f"(80% of observed {_margin_pct:.0%} sell margin).")
    except Exception:
        pass  # no trades yet or older db schema
    auction_only = False
    sort    = "endingSoonest"
    preset  = "8"
