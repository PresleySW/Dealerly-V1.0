...
class Listing:
    ...
    # Image (Sprint 1)
    first_image_url: str = ""
    # Sprint 5: additional gallery URLs (comma-separated, eBay enriched listings only)
    extra_image_urls: str = ""

    # Write-off category (v0.9.10)
    writeoff_category: str = ""         # "Cat S" | "Cat N" | "" (none detected)

    # Auction listing flag (Sprint 15)
    # True when the source platform lists this as an auction (timed bid),
    # not a fixed-price / Buy It Now listing.
...
class DealOutput:
    ...
    p_mot: float
    notes: str
    # Raw median from comp source (before resale_discount × mileage_correction)
    comps_median_raw: Optional[float] = None
