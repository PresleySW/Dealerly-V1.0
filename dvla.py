...
# After HTTP 401/403, skip live DVLA calls for a cooldown to avoid log spam and
# repeated failures when the API key is revoked or quota is exceeded.
_DVLA_COOLDOWN_UNTIL: float = 0.0
_DVLA_AUTH_ERROR_LOGGED: bool = False
_DVLA_COOLDOWN_S: float = 3600.0
...
def dvla_vehicle_enquiry(
    ...
    global _DVLA_COOLDOWN_UNTIL, _DVLA_AUTH_ERROR_LOGGED
    if time.time() < _DVLA_COOLDOWN_UNTIL:
        return None
    ...
    try:
        ...
        if r.status_code in (401, 403):
            if not _DVLA_AUTH_ERROR_LOGGED:
                _DVLA_AUTH_ERROR_LOGGED = True
                print(
                    "  [DVLA] HTTP "
                    f"{r.status_code}: Vehicle Enquiry API rejected this key "
                    "(Forbidden/Unauthorized). Check DVLA_VES_API_KEY and product "
                    "subscription at https://developer-portal.driver-vehicle-licensing.api.gov.uk/ "
                    f"— pausing DVLA calls for {_DVLA_COOLDOWN_S:.0f}s."
                )
            _DVLA_COOLDOWN_UNTIL = time.time() + _DVLA_COOLDOWN_S
            return None
