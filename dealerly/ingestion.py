...
class EbayIngestionAdapter(BaseIngestionAdapter):
    ...
    def fetch_listings(
        ...
        **kwargs,
        ...
        # With 12 queries in preset 7, Ford Fiesta (most listings on eBay)
        # would otherwise swamp the results. 20 per-query gives 240 candidates
        # max across 12 models — enough for scoring without imbalance.
        MAX_PER_QUERY = 20
        batches: List[List[Listing]] = []
        for term in queries:
            try:
                batch = self._fetch_paged(term)
                batches.append(batch[:MAX_PER_QUERY])
