...
def _fetch_query(
    ...
    low_signal_pages = 0
    # Track whether we've already fallen back to the alt URL format for this query
    _url_mode = 0
    ...
    for page_num in range(1, pages + 1):
        ...
        try:
            ...
            if not page_listings:
                ...
                if page_num == 1:
                    # /car-search/ consistently returns 500. Static parsing cannot
                    # extract listings from Motors.co.uk's client-side-rendered HTML.
                    # Go directly to Playwright to execute page JavaScript.
                    print(
                        f"  [motors] Static parsing returned 0 listings "
                        f"— switching to Playwright fallback..."
                    )
                    return listings + _playwright_fetch_query(
                        make, model, price_min, price_max, pages, seen_ids
                    )
                # No results beyond page 1 — stop paginating
                break
...
def _playwright_extract_dom_cards(
    ...
    for card in raw_cards:
        ...
        # Mileage
        mileage: Optional[int] = None
        mileage_match = re.search(r"([\d,]+)\s*(?:miles?|mi\b)", text, re.I)
        if mileage_match:
            try:
...
def _bs4_card_to_listing(
    ...
    # Mileage
    mileage: Optional[int] = None
    mileage_match = re.search(r"([\d,]+)\s*(?:miles?|mi\b)", card.get_text(), re.I)
    if mileage_match:
        try:
            mileage = int(mileage_match.group(1).replace(",", ""))
        except ValueError:
