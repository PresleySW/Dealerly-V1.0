...
from dealerly.config import (
    ...
    DEFAULT_TOP_N,
    MIN_VRM_CONFIDENCE,
    MIN_VRM_CONFIDENCE_FOR_BUY,
    MIN_VRM_CONFIDENCE_MOT_EXCEPTION,
)
...
def fraud_score(
    ...
    if comps_median and comps_median > 0:
        ...
        # Tighter vs median: flag more “too cheap vs comps” cases (salvage/scam/misprice).
        if ratio < 0.32:
            score = max(score, 90)
...
def estimate_resale_and_days(
    *,
    ...
    # 1) AutoTrader — dealer retail prices are ~10-15% above private sale;
    #    apply an extra haircut on top of resale_discount to bridge the gap.
    _AT_RETAIL_HAIRCUT: float = 0.90
    if at_comps:
        at_prices = at_comps.fetch_for_key(
            key, conn, comps_ttl, listing_price=listing.price_gbp
        )
        if len(at_prices) >= 3:
            mv = median(at_prices)
            if mv:
                at_adj = float(mv * resale_discount * _AT_RETAIL_HAIRCUT * mil_corr)
                days = _days_from_mileage(listing.mileage)
                return (
                    at_adj,
                    days,
                    f"resale: AutoTrader median x{resale_discount} x{_AT_RETAIL_HAIRCUT} (retail→private) (n={len(at_prices)}){mil_note}",
                    mv,
                    len(at_prices),
                )
    ...
    # 4) Fallback
    # v1.0 Sprint 26: lowered from 1.35→1.22. Without comps the resale
    # estimate is pure guess — a 35% markup was too optimistic and inflated
    # profit/BUY counts. 1.22x is safer: at £3k buy the fallback resale is
    # £3660, yielding ~£300-400 profit after costs (OFFER territory, not BUY).
    # Real sub-market finds still surface via AT/eBay comps paths above.
    fallback = float(listing.price_gbp * 1.22 * mil_corr)
    days = _days_from_mileage(listing.mileage)
    return fallback, days, f"resale: fallback 1.22x (no comps){mil_note}", None, 0
...
def score_listings(
    ...
    *,
    ...
    for listing in listings:
        ...
        out.comps_median_raw = comps_med

        # v1.0 Sprint 3: enrich generic reason strings with deal-specific numbers
        # so report cards show actionable context rather than boilerplate.
        if out.decision == "BUY" and out.reason == "Meets target margin.":
            _vrm_hint = f" VRM {listing.vrm}." if listing.vrm else ""
            out.reason = (
                f"Est. profit £{out.expected_profit:,.0f} — meets target.{_vrm_hint} "
                f"Inspect + verify before payment."
            )
        elif out.decision == "OFFER" and out.reason == "Profitable only if you negotiate down to max bid.":
            out.reason = (
                f"Negotiate to £{out.max_bid:,.0f} max bid "
                f"(est. £{out.expected_profit:,.0f} net at that price)."
            )
        elif out.decision == "AVOID" and out.reason == "Shock impact too high for current capital.":
            out.reason = (
                f"Repair risk exceeds capital buffer — worst-case "
                f"£{deal.worst_case_repair:,.0f} repairs. Skip or inspect first."
            )
        elif out.decision == "PASS" and out.reason == "Not profitable on current assumptions.":
            out.reason = (
                f"No margin at £{listing.price_gbp:,.0f} asking price. "
                f"Max bid £{out.max_bid:,.0f} for target profit."
            )
        ...
        # v1.0: Confidence-tiered exception — when profit >= 1.5× target AND a
        # high-confidence VRM is confirmed, allow BUY with a "MOT pending" note
        # so the enrichment pipeline can surface actionable decisions before DVSA
        # data arrives. Does NOT weaken the gate for marginal or unverified deals.
        # DVSA MOT missing: do not blanket-downgrade strong BUYs to OFFER when
        # repairs/MOT risk are already in base_rep/worst_rep and economics clear
        # the bar with a usable plate (Sprint 11).
        if mot_provider and out.decision == "BUY" and not listing.mot_history:
            prof = float(out.expected_profit or 0.0)
            vc = listing.vrm_confidence or 0.0
            has_vrm = bool(listing.vrm)
            keep_buy = (
                (prof >= target_margin * 1.5 and has_vrm and vc >= MIN_VRM_CONFIDENCE_MOT_EXCEPTION)
                or (prof >= target_margin * 2.0 and has_vrm and vc >= MIN_VRM_CONFIDENCE)
                or (prof >= target_margin * 1.35 and has_vrm and vc >= MIN_VRM_CONFIDENCE_FOR_BUY)
            )
            if keep_buy:
                out.reason = (
                    f"MOT pending (DVSA) — verify before payment. VRM {listing.vrm} "
                    f"({vc:.0%} conf). Repair/MOT estimates already in numbers."
                )
            else:
                out.decision = "OFFER"
